...
// Configuration-aware fetch function
function timelineFetch(endpoint, options = {}) {
    // Wenn bereits absolute URL verwendet wird, direkt verwenden
    if (endpoint.startsWith('http://') || endpoint.startsWith('https://')) {
        return timelineFetch(endpoint, options);
    }

    // Falls WCIConfig verfügbar ist, verwende es
    if (typeof window !== 'undefined' && window.WCIConfig && window.WCIConfig.initialized) {
        const baseUrl = window.WCIConfig.get('urls.base', '');
        return timelineFetch(baseUrl + endpoint, options);
    }

    // Fallback: verwende relativen Pfad direkt
    return timelineFetch(endpoint, options);
}
...
class TimelineUnifiedRenderer {
    ...
    constructor(containerSelector) {
        ...
        // Tooltip System für Stammdaten-Anzeige
        this.tooltip = {
            element: null,
        ...
        if (Array.isArray(source)) {
            ...
            if (Array.isArray(source.availability)) {
                normalizedAvailability = source.availability.map(entry => ({
                    ...
                    quota_is_multi_day: Boolean(entry?.quota_is_multi_day)
                }));
            }
        }
        ...
        availabilityEntries.forEach(entry => {
            ...
            if (entry?.quota_lager !== undefined || entry?.quota_betten !== undefined ||
                ...
                dailyDetails[index].quota = {
                    ...
                    isMultiDay: Boolean(entry.quota_is_multi_day)
                };
            }
        });
    ...
    setupEvents() {
        ...
        this.canvas.addEventListener('mousemove', (e) => {
            ...
            // Tooltip bleibt fix - KEINE Position-Updates bei Mausbewegung!
        ...
        this.canvas.addEventListener('mouseleave', () => {
            this.hoveredReservation = null;
            this.cancelHoverTimer(); // Tooltip verstecken
    ...
    handleMouseDown(e) {
        ...
        // WICHTIG: Obere 25 Pixel des Histogramms ausschließen (Separator-Bereich)
        const histogramClickableY = this.areas.histogram.y + 25; // Obere 25px für Separator reserviert
        if (mouseY >= histogramClickableY && mouseY <= this.areas.histogram.y + this.areas.histogram.height && mouseX >= this.sidebarWidth) {
            const dayIndex = this.findHistogramDayAt(mouseX);
            if (dayIndex !== null) {
                ...
                if (isMultiDay) {
                    ...
                    if (nextDayIndex < dailyDetails.length) {
                        ...
                        if (nextQuotaData && nextQuotaData.isMultiDay && scaledMax > 0) {
                            ...
                            if (nextFreeCapacity > 0 && scaledMax > 0) {
    ...
    checkHover() {
        ...
        if (this.mouseY >= this.areas.master.y && this.mouseY <= this.areas.master.y + this.areas.master.height) {
            const masterReservation = this.findMasterReservationAt(this.mouseX, this.mouseY);
            this.hoveredReservation = masterReservation;
        }
        // Prüfe Rooms-Bereich
    ...
    findMasterReservationAt(mouseX, mouseY) {
        // Verwende globale reservations Variable (nicht this.reservations!)
        const masterReservations = (typeof reservations !== 'undefined') ? reservations : [];

        if (!masterReservations || masterReservations.length === 0) {
            return null;
        }

        const { startDate } = this.getTimelineDateRange();
        const startX = this.sidebarWidth - this.scrollX;
        const masterY = this.areas.master.y - this.masterScrollY;

        // Filter sichtbare Reservierungen und berechne Positionen
        const visibleReservations = [];

        for (const res of masterReservations) {
            if (!res.start || !res.end) continue;

            const resStart = new Date(res.start);
            const resEnd = new Date(res.end);

            // Setze beide auf Mittag für konsistente Berechnung
            resStart.setHours(12, 0, 0, 0);
            resEnd.setHours(12, 0, 0, 0);

            const duration = (resEnd - resStart) / (1000 * 60 * 60 * 24);
            const startOffset = (resStart - startDate) / (1000 * 60 * 60 * 24);
            const left = startX + (startOffset + 0.1) * this.DAY_WIDTH;
            const width = (duration - 0.2) * this.DAY_WIDTH;

            // Nur sichtbare Reservierungen
            if (left + width > this.sidebarWidth && left < this.canvas.width) {
        ...
        // Stacking-Berechnung (gleich wie beim Rendering)
        const OVERLAP_TOLERANCE = this.DAY_WIDTH * 0.1;
        const sortedReservations = visibleReservations.sort((a, b) => a.startOffset - b.startOffset);
        ...
        sortedReservations.forEach((res, index) => {
            ...
            while (!placed) {
                ...
                for (let i = 0; i < index; i++) {
                    const other = sortedReservations[i];
                    if (other.stackLevel === stackLevel) {
                        const resEnd = res.left + res.width;
                        const otherEnd = other.left + other.width;

                        if (!(resEnd <= other.left + OVERLAP_TOLERANCE ||
                            res.left >= otherEnd - OVERLAP_TOLERANCE)) {
                            canPlaceHere = false;
                            break;
                        }
                    }
        ...
        // Suche getroffene Reservierung
        const barHeight = this.MASTER_BAR_HEIGHT || 16;
        const edgeHandleMargin = 6;

        for (const res of sortedReservations) {
            const y = masterY + 5 + (res.stackLevel * (barHeight + 2));

            const isInXRange = mouseX >= res.left - edgeHandleMargin && mouseX <= res.left + res.width + edgeHandleMargin;
            const isInYRange = mouseY >= y - edgeHandleMargin && mouseY <= y + barHeight + edgeHandleMargin;
    ...
    initTooltip() {
        ...
        tooltipEl.className = 'tooltip bs-tooltip-top fade';
        ...
        document.body.addEventListener('click', (e) => {
            if (e.target.classList.contains('tooltip-email-copy')) {
                ...
                // Klick innerhalb E-Mail-Bereich - nicht schließen
                e.stopPropagation();
            } else if (this.tooltip.isVisible && !tooltipEl.contains(e.target)) {
                // Klick außerhalb Tooltip (außer E-Mail-Felder) - schließen
                this.hideTooltip();
            }
        });
    ...
    toggleTooltipAtMouse() {
        if (this.tooltip.isVisible) {
            // Tooltip ist sichtbar - schließen
            this.hideTooltip();
            return;
        }
        ...
        // Prüfe Master-Bereich (berücksichtige, dass wir nur prüfen ob Maus visuell im Master-Bereich ist)
        // Die masterScrollY wird in findMasterReservationAt intern berücksichtigt
        if (this.mouseY >= this.areas.master.y && this.mouseY <= this.areas.master.y + this.areas.master.height) {
    ...
    async loadMasterData() {
        try {
            ...
            const response = await timelineFetch(`/wci/zp/getReservationMasterData.php?start=${startDateStr}&end=${endDateStr}`);
            const result = await response.json();

            if (result.success) {
                this.tooltip.masterDataLookup = result.data;
                console.log(`✅ Stammdaten geladen: ${Object.keys(result.data).length} Reservierungen`);
            }
        } catch (error) {
    ...
    showTooltip(reservation) {
        if (!reservation) {
            console.warn('⚠️ showTooltip: Keine Reservierung übergeben');
            return;
        }

        // Ermittle res_id aus verschiedenen Quellen (erweitert für Master-Reservierungen)
        const resId = reservation.res_id ||
            reservation.resid ||
            reservation.reservation_id ||
            reservation.data?.res_id ||
            reservation.data?.id ||
        ...
        console.log('🔍 showTooltip für:', {
            resId,
            name: reservation.name || reservation.guest_name,
            hasData: !!reservation.data,
            keys: Object.keys(reservation)
        });

        if (!resId) {
            console.warn('⚠️ Keine res_id gefunden in:', reservation);
            return;
        }

        // Hole Stammdaten aus Lookup
        const masterData = this.tooltip.masterDataLookup[resId];
        if (!masterData) {
            console.warn(`⚠️ Keine Stammdaten gefunden für res_id: ${resId}`, {
                availableIds: Object.keys(this.tooltip.masterDataLookup).slice(0, 5),
                totalCount: Object.keys(this.tooltip.masterDataLookup).length
            });
            return;
        }

        console.log('✅ Stammdaten gefunden:', masterData);

        // Formatiere Tooltip-Inhalt
        const tooltipHTML = this.formatTooltipContent(masterData);

        const tooltipInner = this.tooltip.element.querySelector('.tooltip-inner');
        tooltipInner.innerHTML = tooltipHTML;
    ...
    formatTooltipContent(data) {
        ...
        const capacityText = capacityParts.length > 0 ? capacityParts.join(', ') : 'Keine Angabe';
        ...
        // E-Mail-Betreff für mailto: "ihr aufenthalt vom xx - yy (ResNr: id/av_id)"
        const resNr = data.av_id > 0 ? `${data.id}/${data.av_id}` : `${data.id}`;
        const emailSubject = `Aufenthalt vom ${data.anreise} - ${data.abreise} (ResNr: ${resNr})`;
        const emailSubjectEncoded = encodeURIComponent(emailSubject);

        return `
            <div style="line-height: 1.5;">
                <div style="font-weight: bold; font-size: 14px; margin-bottom: 8px; border-bottom: 1px solid #333; padding-bottom: 6px; color: #000;">
                    ${fullName}${gruppenname ? ` <span style="color: #333;">(${gruppenname})</span>` : ''}
                </div>
                
                <div style="margin-bottom: 8px; font-size: 12px;">
                    <div style="display: flex; align-items: center; gap: 4px; margin-bottom: 4px;">
                        ${email !== 'Keine E-Mail' ? `
                            <span class="tooltip-email-copy" data-email="${email}" style="cursor: pointer; color: #0066cc; text-decoration: none;" title="In Zwischenablage kopieren">${email}</span>
                            <a href="mailto:${email}?subject=${emailSubjectEncoded}" style="text-decoration: none; font-size: 16px;" title="E-Mail senden">✉️</a>
                        ` : `<span style="color: #666;">${email}</span>`}
                    </div>
                    <div style="color: #000;">${telefon}</div>
                </div>
                
                <div style="margin-bottom: 8px; padding-top: 6px; border-top: 1px solid #ccc; font-size: 12px; color: #000;">
                    <div><strong>Anreise:</strong> ${data.anreise} &nbsp;|&nbsp; <strong>Abreise:</strong> ${data.abreise}</div>
                    <div><strong>Arrangement:</strong> ${arrangement}</div>
                    <div><strong>Gesamt:</strong> ${totalCapacity} Person${totalCapacity !== 1 ? 'en' : ''}${hund} <span style="color: #333;">(${capacityText})</span></div>
                </div>
                
                <div style="padding-top: 6px; border-top: 1px solid #ccc; font-size: 12px; color: #000;">
                    <div style="margin-bottom: 4px;"><strong>Herkunft:</strong> ${herkunft}</div>
                    ${bemerkungIntern !== '-' ? `<div style="margin-bottom: 4px;"><strong>Bemerkung intern:</strong> <span style="font-style: italic; color: #222;">${bemerkungIntern}</span></div>` : ''}
                    ${bemerkungAV !== '-' ? `<div><strong>Bemerkung AV:</strong> <span style="font-style: italic; color: #222;">${bemerkungAV}</span></div>` : ''}
                </div>
            </div>
        `;
    }
    ...
    positionTooltip() {
        ...
        if (top < padding) {
            // Wenn nicht genug Platz oben, dann unten anzeigen
        ...
        try {
            ...
            if (result.success && result.data) {
                ...
                result.data.forEach(item => {
                    if (item.id) this.tooltip.masterDataLookup[item.id] = item;
                    if (item.av_id) this.tooltip.masterDataLookup[item.av_id] = item;
                });
            }
        } catch (error) {
            console.error('Fehler beim Laden der Stammdaten:', error);
