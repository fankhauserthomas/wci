    // Koordiniertes Refresh-System (arbeitet mit script.js zusammen)
    let autoRefreshInterval = null;
    let autoRefreshEnabled = false;
    let scriptJsIsLoading = false;
    let lastTableRebuild = 0;function startAutoRefresh() {
  ...
  console.log('🔄 Auto-Refresh gestartet (läuft im Hintergrund alle 10 Sekunden)');
      ...
      autoRefreshInterval = setInterval(async () => {
        if (!autoRefreshEnabled || scriptJsIsLoading) {
          console.log('🔄 Auto-Refresh pausiert (Tab inaktiv oder script.js lädt)');
          return;
        }
        
        // Prüfe ob script.js gerade Tabelle neu erstellt
        if (Date.now() - lastTableRebuild < 5000) {
          console.log('🔄 Auto-Refresh übersprungen (Tabelle kürzlich neu erstellt)');
          return;
        }
        
        try {
          console.log('🔄 Auto-Refresh: Koordiniertes Update...');
          
          // 1. Benachrichtige script.js dass wir ein Update machen
