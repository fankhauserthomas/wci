<?php
// get-hp-arrangements-header-debug.php - Debug-Version ohne Auth für Tests
require_once 'hp-db-config.php';

// Temporärer Bypass der Authentifizierung für Debug-Zwecke
...
try {
    ...
    error_log("DEBUG: Getting HP arrangements for header display - res_id: $resId");
    ...
    if ($arrResult) {
        ...
        error_log("DEBUG: Found " . count($availableArrangements) . " available arrangements in hparr table");
    }
    ...
    while ($guest = $guestsResult->fetch_assoc()) {
        ...
        error_log("DEBUG: Processing guest: ID=$guestId, Name='$guestName'");
        ...
        if (!$arrStmt) {
            error_log("DEBUG: Could not prepare hpdet query: " . $hpConn->error);
            continue;
        }
        ...
        while ($arr = $arrResult->fetch_assoc()) {
            ...
            error_log("DEBUG: Found arrangement: ID=$arrId, Name='$arrName', Count=$anz, Remark='$bem', Guest='$guestName', TimeClass='$timeClass'");
    ...
    error_log("DEBUG: Returning " . count($arrangementsArray) . " arrangement types with total $totalItems items for $guestCount guests");
    ...
    error_log("DEBUG: Get HP arrangements header error: " . $e->getMessage());
