...
/**
 * Generiert ein PNG-Bild für einen Tisch (nur wenn GD verfügbar)
 */
...
function getTischNamenUebersicht() {
    ...
    try {
        ...
        // Konvertiere zu gewünschtem Format und generiere PNGs/Vorschauen
        foreach ($tischGruppen as $tischName => $namen) {
            // Dateiname für PNG
            $safe_tisch_name = preg_replace('/[^a-zA-Z0-9_-]/', '_', $tischName);
            $png_filename = $safe_tisch_name . '.png';
            $png_path = __DIR__ . '/erd/' . $png_filename;
            
            // PNG generieren (nur wenn GD verfügbar)
            $png_generated = false;
            $png_exists = false;
            $use_html_preview = false;
            
            if (isGDAvailable()) {
                $png_generated = generateTischPNG($tischName, $namen, $png_path);
...
<head>
    ...
    <style>
        * {
        ...
        /* HTML-Vorschau als Alternative zu PNG */
        .html-preview {
            width: 120px;
            height: 68px;
            border: 2px solid #dee2e6;
...
<body>
        ...
        <?php else: ?>
            ...
            <?php if (!isGDAvailable()): ?>
                <div class="gd-status error">
                    ⚠️ <strong>GD Extension nicht verfügbar:</strong> PNG-Generierung deaktiviert. Verwende HTML-Vorschau als Alternative.
                    <br><small>Für PNG-Support: <code>sudo apt-get install php-gd</code> und Webserver neu starten.</small>
                </div>
            <?php else: ?>
                <div class="gd-status success">
                    ✅ <strong>GD Extension aktiv:</strong> PNG-Generierung verfügbar.
                </div>
            <?php endif; ?>
                    ...
                    <tbody>
                        <?php foreach ($tischNamenData as $row): ?>
                            <tr>
                                ...
                                <!-- Vorschau (PNG oder HTML-Alternative) -->
                                <td class="vorschau-cell">
                                    ...
                                    <?php elseif ($row['png_exists'] && $row['png_generated']): ?>
                                        <!-- PNG-Vorschau -->
                                        <img src="erd/<?php echo htmlspecialchars($row['png_filename']); ?>?<?php echo time(); ?>" 
                                             alt="Tisch <?php echo htmlspecialchars($row['tisch']); ?>" 
                                             class="preview-thumbnail"
                                             onclick="showImageModal('erd/<?php echo htmlspecialchars($row['png_filename']); ?>', '<?php echo htmlspecialchars($row['tisch']); ?>')">
                                        <br>
                                        <button class="preview-button" 
                                                onclick="downloadImage('erd/<?php echo htmlspecialchars($row['png_filename']); ?>', '<?php echo htmlspecialchars($row['tisch']); ?>')">
                                            💾 PNG
                                        </button>
                                    <?php elseif ($row['png_generated'] === false): ?>
                                        <!-- PNG-Fehler -->
    ...
    <script>
        ...
        function downloadImage(imagePath, tischName) {
            ...
            link.href = imagePath;
            link.download = 'Tisch_' + tischName + '.png';
